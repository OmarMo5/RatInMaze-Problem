...
public class MazeSolver extends Thread{
    ...
    MazeSolver( int[][] matrice,int[] start,String label){
        ...
        this.label=label;
        MazeSolver.counter--;
        tt.setBounds(20, 20, 650, 650); //this board
    ...
    public void run(){
        ...
        int fin=1;
        while(fin==1 && !JavaApplication1.goalAchieved){
            ...
            fin=this.move();
            if(  this.xPos==JavaApplication1.exit[0]  && this.yPos==JavaApplication1.exit[0] ) {
                JavaApplication1.setStatic();
                System.out.println("Destnation ...");
                MazeSolver.matrice[this.xPos][this.yPos]=2; //when we arrive the point require
            }
            if(fin!=1 && !JavaApplication1.goalAchieved ){
                MazeSolver.matrice[this.xPos][this.yPos]=-22; //when we walk in the path that found 0
            }
